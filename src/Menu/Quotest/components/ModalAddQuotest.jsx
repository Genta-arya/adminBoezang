...
import { postQuotest } from "../../../services/Quotest/ApiQuotest";
import { message } from "antd";
import { useNavigate } from "react-router-dom";

const ModalAddQuotest = ({ isOpen, onClose, refresh }) => {
  ...
  const handleSave = async (event) => {
    ...
    try {
      const response = await postQuotest(quoteData);
      message.success("Quotes Berhasil ditambahkan");
      onClose(false);
      refresh();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      >
        <h2 className="text-2xl font-bold mb-4">Tulis Quotes</h2>
...
export default ModalAddQuotest;
